...
import {StatusMap, ProductMap} from '../../utils/mappers';
  ...
  return (
    <React.Fragment>
      <StyledTableRow open={open} onClick={() => setOpen(!open)}>
        ...
        <TableCell align="right">{row.rto}</TableCell>
...
export const ProductTableMarketing = ({products}: any) => {
  const [modalOpen, setModalOpen] = useState(false); // NOTE: открытие модалки
  const [modalData, setModalData] = useState({
    task_id: -1,
  }); // NOTE: открытие модалки
